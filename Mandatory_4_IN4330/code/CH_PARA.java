...
public class CH_PARA {
    ...
    public static void main(String[] args)throws InterruptedException{
        ...
        try {
            ...
            "java Oblig3 <n> <seed> <k> where <n> <seed> <k> are positive integers. n must be greater than 2.");
            return;
        }
        ...
        double[] Time_seq = new double[runs]; 
        double[] Time_para = new double[runs];
        for (int run = 0; run < runs; run++){
            ...
            AreEqual(convexHull_seq, convexHull_seq);
            ...
            System.out.println("ferdig med UP");
            ...
            System.out.println("Time used Parallel: " + ElapsedTimePara);
            
            Time_seq[run] = ElapsedTimeseq;
            Time_para[run] = ElapsedTimePara;
            if (n < 10000 && run == 0){
                Oblig4Precode p = new Oblig4Precode(convex, convexHull_para);
                p.writeHullPoints(); //write to file
                p.drawGraph(); //draw graph
                convexHull_para.print(); //output in terminal
            }
    
            AreEqual(convexHull_seq, convexHull_para);
            System.out.println("\n");
        }
        if (runs == 7){
            Arrays.sort(Time_seq);
            Arrays.sort(Time_para);
            System.out.println("Median time Sequential: " + Time_seq[3]);
            System.out.println("Median time Parallel: " + Time_para[3]);
            System.out.println("Speedup: "  + Time_seq[3]/Time_para[3]);
        }
    }

    /*
     * Method to test if two IntLists are equal.
     * We use this to check if the sequential solution and parallel solution are equal.
     */
    public static void AreEqual(IntList seq, IntList para){
        int length = seq.size();
        for (int i = 0; i < length; i++){
            if (seq.get(i) != para.get(i)){
